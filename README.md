📚 Repositório de Atividades e Projetos — SENAI

👨‍💻 Olá! Eu sou Joesley Cálita, aluno do SENAI.

Este repositório foi criado com o objetivo de organizar, documentar e acompanhar minha evolução durante o curso. Aqui serão adicionadas atividades, exercícios, projetos e outros trabalhos desenvolvidos ao longo da formação.

🎯 Objetivo: Registrar minha jornada de aprendizado, colocando em prática os conhecimentos adquiridos durante as aulas e acompanhando minha evolução profissional e técnica.

📂 Conteúdo do Repositório

Ao longo do curso, este repositório poderá conter:

📝 Atividades e exercícios;

💻 Projetos práticos;

🧠 Desafios e atividades de aprendizagem;

📖 Trabalhos realizados durante o curso;

🔧 Aplicações dos conhecimentos adquiridos em aula;

📈 Projetos desenvolvidos individualmente ou em equipe;

🚀 Evolução;

Este repositório estará em constante atualização conforme o andamento do curso. A ideia é que ele sirva não apenas como um espaço para armazenar atividades, mas também como um registro da minha evolução e dos conhecimentos adquiridos durante minha formação no SENAI.

"O aprendizado é um processo contínuo. Cada projeto é uma oportunidade para aprender algo novo."

🏫 SENAI

📚 Curso: Desenvolvimento de Sistemas

📅 Período: 2026.2

⭐ Este repositório será atualizado ao longo do curso com novos projetos e atividades.
